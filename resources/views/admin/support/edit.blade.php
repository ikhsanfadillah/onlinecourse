...
@section('content')
    ...
    <!-- form start -->
    @if(isset($supportDesk))
    <form action="{{ route('support.update', $supportDesk->id) }}" method="post">
      @csrf
      @method('PUT')
      <div class="row">
          <div class="col-12">
              ...
              <div class="card card-success card-outline">
                  ...
                  <div class="card-body">
                      <div class="form-group">
                          ...
                          <input required id="txtTitle" type="text" name="title" value="{{ $supportDesk->title }}" class="form-control @error('title') is-invalid @enderror" placeholder="Enter Title" >
                          @error('title')
                      ...
                      <div class="form-group">
                          ...
                          <input required value="{{$supportDesk->slug}}" id="txtSlug" type="text" name="slug" class="form-control @error('slug') is-invalid @enderror" placeholder="Customize Slug" >
                          @error('slug')
                          <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                          @enderror
                      </div>
                      ...
                      <div class="form-group">
                          ...
                          <select required name="topic" id="" class="form-control">
                            <option value="privacy_policy" @if ($supportDesk->topic == 'privacy_policy') selected @endif>Privacy Policy</option>
                            <option value="terms_and_conditions" @if ($supportDesk->topic == 'terms_and_conditions') selected @endif>Terms & Condition</option>
                          </select>
                          @error('topic')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                          @enderror
                      ...
                      <div class="form-group">
                        ...
                        <textarea required name="content" id="content">{{ $supportDesk->content }}</textarea>
                        @error('content')
                          <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                      </div>
